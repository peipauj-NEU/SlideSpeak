...
class SlideSpeakGUI:
    def __init__(self, root):
        ...
        self.style.configure('TLabel', font=(
            'Arial', 12), background='#f5f5f5')
        self.style.configure('Header.TLabel', font=(
            'Arial', 16, 'bold'), background='#f5f5f5')
    ...
    def create_widgets(self):
        ...
        header_label = ttk.Label(
            main_frame, text="SlideSpeak - AI PowerPoint Generator", style='Header.TLabel')
        ...
        input_frame = ttk.LabelFrame(
            main_frame, text="Presentation Topics", padding="10 10 10 10")
        ...
        self.topics_entry = tk.Text(
            input_frame, height=3, width=50, font=('Arial', 12))
        self.topics_entry.pack(fill=tk.X, pady=(0, 10))

        # Bind events to check cache when user stops typing
        self.topics_entry.bind("<KeyRelease>", lambda e: self.root.after(
            500, self.check_cache_for_topic))

        # Add a button to browse cached presentations
        cache_button = ttk.Button(
            input_frame, text="Browse Cached Presentations", command=self.browse_cache)
        cache_button.pack(anchor=tk.E, pady=(0, 5))

        # Options section
        options_frame = ttk.LabelFrame(
            main_frame, text="Generation Options", padding="10 10 10 10")
        ...
        self.save_path_var = tk.StringVar(
            value=os.path.join(os.getcwd(), "PPT.pptx"))
        save_entry = ttk.Entry(
            save_frame, textvariable=self.save_path_var, width=50)
        ...
        log_frame = ttk.LabelFrame(
            main_frame, text="Status Log", padding="10 10 10 10")
    ...
    def check_cache_for_topic(self):
        ...
        if cached_result:
            self.cache_status.config(
                text=f"✓ Cached version available for '{main_topic}'", foreground="green")
        else:
            self.cache_status.config(text="")
    ...
    def get_cache_folder_for_idea(self, idea):
        ...
        sanitized_idea = "".join(
            c if c.isalnum() or c in "._- " else "_" for c in idea)
        ...
        for folder in os.listdir(cache_dir):
            ...
            if os.path.isdir(folder_path):
                ...
                if len(parts) > 1:  # Make sure there's at least one part before the hash
                    ...
                    normalized_folder_idea = self.normalize_idea_name(
                        folder_idea)
                    ...
                    if normalized_folder_idea == normalized_idea:
                        ...
                        outline_path = os.path.join(
                            folder_path, "enriched_outline.json")
                        speech_path = os.path.join(
                            folder_path, "presentation_speech.md")
    ...
    def generate_presentation(self):
        ...
        threading.Thread(target=self._generate_presentation_thread,
        ...
        try:
            ...
            if cached_result:
                ...
                with open("./output/enriched_outline.json", "w", encoding="utf-8") as file:
                    json.dump(enriched_outline, file,
    ...
    def open_tts_selector(self):
        ...
        btn_pyttsx3.pack(side=tk.LEFT, padx=(
            0, 5), pady=5, expand=True, fill=tk.X)

        btn_kokoro = ttk.Button(button_frame, text="Kokoro (Neural TTS)",
                                command=lambda: self.launch_tts_engine("kokoro"))
        btn_kokoro.pack(side=tk.LEFT, padx=(5, 0),
                        pady=5, expand=True, fill=tk.X)
        ...
        for folder in os.listdir(cache_dir):
            ...
            if not self.is_valid_cache_folder(folder_path):
                try:
                    ...
                    print(
                        f"Error removing invalid cache folder {folder}: {str(e)}")
    ...
    def browse_cache(self):
        ...
        try:
            ...
            messagebox.showerror(
                "Error", f"Failed to read cache directory: {str(e)}")
            return

        if not cached_ideas:
            messagebox.showinfo(
                "Cache Empty", "No cached presentations found.")
            return
        ...
        ttk.Label(frame, text="Select a cached presentation:").pack(
        ...
        # Create the listbox
        listbox = tk.Listbox(
            list_frame, yscrollcommand=scrollbar.set, font=('Arial', 12), height=10)
        ...
        def load_selected_presentation():
            ...
            if selection:
                ...
                outline_path = os.path.join(
                    selected_idea["folder_path"], "enriched_outline.json")
                speech_path = os.path.join(
                    selected_idea["folder_path"], "presentation_speech.md")
                ...
                try:
                    ...
                    self.log(
                        f"Loaded cached presentation: {selected_idea['display_name']}")
                    ...
                    # Success message
                    messagebox.showinfo(
                        "Success", "Cached presentation loaded successfully!")
            ...
            if selection:
                ...
                if messagebox.askyesno("Confirm Delete", f"Are you sure you want to delete the cached presentation:\n{selected_idea['display_name']}?"):
                    try:
                        ...
                        self.log(
                            f"Deleted cached presentation: {selected_idea['display_name']}")
                        ...
                        if not cached_ideas:
                            messagebox.showinfo(
                                "Cache Empty", "No more cached presentations available.")
                            cache_window.destroy()
                    except Exception as e:
                        messagebox.showerror(
        ...
        load_button = ttk.Button(
            button_frame, text="Load Selected", command=load_selected_presentation)
        load_button.pack(side=tk.LEFT, padx=(0, 5))

        delete_button = ttk.Button(
            button_frame, text="Delete Selected", command=delete_selected_presentation)
        delete_button.pack(side=tk.LEFT)

        cancel_button = ttk.Button(
            button_frame, text="Cancel", command=cache_window.destroy)
    ...
    def launch_tts_engine(self, engine_type):
        ...
        try:
            ...
            messagebox.showerror(
                "Error", f"Failed to launch TTS engine: {str(e)}")
            self.log(f"Error launching TTS engine: {str(e)}")


def main():
    root = tk.Tk()
    app = SlideSpeakGUI(root)
    root.mainloop()
